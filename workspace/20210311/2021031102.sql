...
예) 1~50사이에 존재하는 FIBONACCI NUMBER를 출력하시오
    첫 째, 둘 째 값은 1,1로 주어지고 그 다음부터 전 두수의 합이 현재수가 됨
DECLARE
    PP_NUM NUMBER:=1;
    P_NUM NUMBER:=1;
    C_NUM NUMBER:=0;
...
***PL/SQL에서 반복문은 주로 커서를 사용할 때 사용
    CURSOR
      . SQLDML DML명령으로 영향받은 행의 집합
      . 묵시적커서와 명시적 커서가 존재
      (1) 묵시적커서
         - 사용자가 SELECT문을 수행시켰을 때 생성되는 커서
         - 이름이 없음
         - 항상 CLOSE상태임(내부  데이터 접근 불가능)
         커서속성
         SQL%FOUND : 커서 내에 자료(행)가 존재하면 참을 반환
         SQL%NOTFOUND : 커서 내에 자료(행)가 존재하지 않으면 참을 반환
         SQL%ISOPEN : 커서가 OPEN되어 있으면 참을 반환(항상 FALSE)
         SQL%ROWCOUNT : 커서집합내에 레코드(행)의 수 반환
      (2) 명시적커서
...
사용예) 2005년 6월 상품을 구매한 고객LIST를 출력하는 커서작성
       고객LIST는 고객번호, 고객명, 주소, 전화번호이다.
DECLARE
...
END;
    ...
    (나)커서의 OPEN
      . 명시적 커서의 사용을 위해 커서를 반드시 OPEN해야함
      . BEGIN BLOCK에서 수행
      (사용형식)
      OPEN 커서명[(값LIST)]
...
BEGIN
    ...
    LOOP
        ...
        SELECT MEM_NAME, MEM_ADD1||' '||MEM_ADD2, MEM_HP
          INTO V_NAME, V_ADDR, V_TEL
          FROM MEMBER
         WHERE MEM_ID = V_ID;
